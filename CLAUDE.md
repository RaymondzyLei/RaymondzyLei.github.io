...
- **Scroll-reveal（`useReveal`）**：6 个 section + 区块内卡片错位渐现。opacity 0→1 + translateY(24px)→0，缓动 `cubic-bezier(0.22, 1, 0.36, 1)`，600ms。`useReveal` 包装 `react-intersection-observer` 的 `useInView`（threshold 0.1、rootMargin `'0px 0px -10% 0px'`、triggerOnce true）+ MUI `useMediaQuery`；`prefers-reduced-motion: reduce` 时直接 `isVisible: true` 无 observer。错位 stagger：同区块卡片 `transitionDelay: ${index * 100}ms`，由组件自己写（在 sx 里），hook 不传 delay。
